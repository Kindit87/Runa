...
public class TrackInfo extends Subcommand {
    ...
    public void interaction(SlashCommandInteractionEvent event) {
        ...
        if (scheduler.audioPlayer.getPlayingTrack() != null) {
            event
                .getHook()
                .sendMessageEmbeds(getEmbed(scheduler))
                .setComponents(
                    ActionRow.of(
    ...
    private MessageEmbed getEmbed(TrackScheduler scheduler) {
        ...
        if (scheduler.audioPlayer.getPlayingTrack() != null) {
            ...
            eb.setTitle(
                scheduler.audioPlayer.getPlayingTrack().getInfo().title,
                scheduler.audioPlayer.getPlayingTrack().getInfo().uri
            );
            eb.addField(
                "Объектов в очереди",
                String.valueOf(scheduler.getQueue().size() + 1),
                true
            );
            eb.addField(
                "Громкость",
                scheduler.audioPlayer.getVolume() + "%",
                true
            );
