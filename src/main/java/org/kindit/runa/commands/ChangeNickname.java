package org.kindit.runa.commands;
...
public class ChangeNickname extends Command {
    ...
    public void interaction(SlashCommandInteractionEvent event)
        ...
        boolean callerCanChangeNickname = Objects.requireNonNull(
            event.getMember()
        ).hasPermission(Permission.NICKNAME_CHANGE);
        Member member = Objects.requireNonNull(
            event.getOption("user")
        ).getAsMember();
        boolean isBot = Objects.requireNonNull(member)
            .getUser()
            .equals(event.getJDA().getSelfUser());
        String newNickname = Objects.requireNonNull(
            event.getOption("nickname")
        ).getAsString();
        ...
        if (isBot) {
            event
                .getHook()
                .sendMessageEmbeds(
                    Command.notSuccessfullyReplyEmbed("I won't do this.")
                )
                .queue();
            return;
            ...
            event
                .getHook()
                .sendMessageEmbeds(
                    Command.notSuccessfullyReplyEmbed(
                        "You do not have rights to change nicknames."
                    )
                )
                .queue();
            return;
        ...
        try {
            ...
            event
                ...
                .sendMessageEmbeds(
                    Command.notSuccessfullyReplyEmbed(
                        "Can't modify a member with higher or equal highest role than bot."
                    )
                )
                .queue();
            return;
            ...
            event
                ...
                .sendMessageEmbeds(
                    Command.notSuccessfullyReplyEmbed(
                        "Bot do not have rights to change nicknames."
                    )
                )
                .queue();
            return;
