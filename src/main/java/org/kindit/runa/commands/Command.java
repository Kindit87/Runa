...
public abstract class Command {
    ...
    protected Command(String name, String description) {
        ...
        this.userName = JsonConfig.getInstance()
            .getCommand(name)
            .get("name")
            .toString();
        this.active = Boolean.parseBoolean(
            JsonConfig.getInstance().getCommand(name).get("active").toString()
        );
    }
    ...
    public CommandData getCommandData() {
        if (subcommands != null) {
            return Commands.slash(userName, description).addSubcommands(
    ...
    public void interaction(SlashCommandInteractionEvent event)
        ...
        for (Subcommand command : subcommands) {
            if (
                event
                    .getFullCommandName()
                    .equals(userName + " " + command.userName)
            ) {
    ...
    protected static MessageEmbed replyEmbed(
        String title,
        String description,
        Color color
    ) {
